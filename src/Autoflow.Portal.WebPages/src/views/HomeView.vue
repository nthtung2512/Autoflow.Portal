...
const getSenderConversationsEntry = ref(userConversation.filter((usconv) => usconv.userId === senderUserId))
const getSenderConversations = ref(getSenderConversationsEntry.value.map((item) => item.conversationId))
...
const getReceiverEntry = ref(
  getSenderConversationsEntry.value.map((item) => 
    userConversation.filter((usconv) => 
...
const receiverUsers = ref<User[]>(getReceiverUserIds.value.map(
...
watch(selectedReceiver, (newValue, oldValue) => {
  ...
  const selectedConversationId = getReceiverEntry.value.find((usconv) => usconv.userId === newValue.userId)?.conversationId
  selectedConversation.value = conversations.find((conv) => conv.conversationId === selectedConversationId)
...
const sendMessage = (sendUserId: UUID, receiveUserId: UUID, content: string, conversationId: UUID) => {
  ...
  if (conversationId === null) {
    ...
    conversationsStore.postMessageToConversation(selectedConversation.value?.conversationId, newMessage)
  }
  else {
    console.log("Check 2")
  ...
  console.log('Send messages from ', sendUserId, ' to receiver:', receiveUserId, ' with message: ', content)
...
watch(
  ...
  () => {
    // Handle the reload logic here, like fetching data again
      // Get current sender user
    getSenderConversationsEntry.value = userConversation.filter((usconv) => usconv.userId === senderUserId)
    getSenderConversations.value = getSenderConversationsEntry.value.map((item) => item.conversationId)
    getReceiverEntry.value = getSenderConversationsEntry.value.map((item) => 
      userConversation.filter((usconv) => 
        usconv.conversationId === item.conversationId && usconv.userId !== senderUserId
      )[0]
    )
